...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, shared_focal=False, flow_loss_fn='smooth_l1', flow_loss_weight=0.0, 
        ...
        #     [self.conf_trf(self.conf_i[i_j]) * (1-self.im_dyna[int(i_j.split('_')[0])]) for i_j in self.str_edges], fill=self.max_area))
        # self.register_buffer('_weight_j', ParameterStack(
        #     [self.conf_trf(self.conf_j[i_j]) * (1-self.im_dyna[int(i_j.split('_')[1])]) for i_j in self.str_edges], fill=self.max_area))
