...
class BasePCOptimizer (nn.Module):
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
        ...
        self.use_self_mask = not (use_gt_mask or use_pred_mask)
        self.pred_motion_mask_thre = pred_motion_mask_thre

        self.i_count = {}
        self.j_count = {}
        for i, j in zip(view1['idx'], view2['idx']):
            self.i_count[i] = self.i_count.get(i, 0) + 1
            self.j_count[j] = self.j_count.get(j, 0) + 1
        ...
        for i in range(len(self.im_conf)):
            # self.im_conf[i].requires_grad = True
            self.im_conf[i].requires_grad = False

        self.init_conf_maps = [c.clone() for c in self.im_conf]

        if use_pred_mask and not use_gt_mask:
            aggreagte_mode = 'avg'
            # print("Aggreagating predicted dynamic masks, mode: ", aggreagte_mode)
            # compute image dynamic masks
            pred_1_mmask = pred1['dynamic_mask']
            self.mmask_n = {nm: pred_1_mmask[i] for i, nm in enumerate(self.str_edges)}
            self.im_dyna = self._compute_img_mmask(pred_1_mmask, reduce = aggreagte_mode)
            self.im_dyna_avg = self._compute_img_mmask(pred_1_mmask, reduce = 'avg')
            self.im_dyna_max = self._compute_img_mmask(pred_1_mmask, reduce = 'max')
        ...
        for v in range(len(self.edges)):
            idx = view1['idx'][v]
            dynamic_masks[idx] = self.im_dyna[idx] > self.pred_motion_mask_thre
            idx = view2['idx'][v]
            dynamic_masks[idx] = self.im_dyna[idx] > self.pred_motion_mask_thre
    ...
    @torch.no_grad()
    def _compute_img_mmask(self, pred1_mmask, reduce = 'avg'):
        im_dyna = [torch.zeros(hw, device=self.device) for hw in self.imshapes]
        if reduce == 'avg':
            for e, (i, j) in enumerate(self.edges):
                im_dyna[i] = torch.add(im_dyna[i], pred1_mmask[e]/self.i_count[i])
                # im_dyna[j] = torch.add(im_dyna[j], pred2_mmask[e]/self.j_count[j])
        if reduce == 'max':
            for e, (i, j) in enumerate(self.edges):
                im_dyna[i] = torch.maximum(im_dyna[i], pred1_mmask[e])
                # im_dyna[j] = torch.maximum(im_dyna[j], pred2_mmask[e])
        return im_dyna
    ...
    def save_dyna_maps(self, path):
        dyna_avg = self.im_dyna_avg
        for i, c in enumerate(dyna_avg):
            np.save(f'{path}/dyna_avg_{i}.npy', c.detach().cpu().numpy())

        dyna_max = self.im_dyna_max
        for i, c in enumerate(dyna_max):
            np.save(f'{path}/dyna_max_{i}.npy', c.detach().cpu().numpy())
