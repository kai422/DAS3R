...
def eval_metrics(pred_traj, gt_traj=None, seq="", filename="", sample_stride=1):
    ...
    return ate_result, ate, rpe_trans, rpe_rot
...
def best_plotmode(traj):
    ...
    return getattr(plot.PlotMode, 'xyz')
...
def plot_trajectory(
    ...
    plt.rcParams['lines.markersize'] = 1.25
    ax.set_title(title)
    if gt_traj is not None:
    ...
    fig.canvas.draw()
    image = np.frombuffer(fig.canvas.tostring_argb(), dtype='uint8').astype(np.float32) / 255.0
